...
final class ChatListViewModelTests: XCTestCase {
    ...
    func testChatListDate() throws {
        repo.chatListStub = makeChatList()
        let sut = try XCTUnwrap(sut)
        XCTAssertEqual(sut.chatList[0].timestamp, Date(timeIntervalSince1970: 100).description)
        XCTAssertEqual(sut.chatList[1].timestamp, Date(timeIntervalSince1970: 200).description)
        XCTAssertEqual(sut.chatList[2].timestamp, Date(timeIntervalSince1970: 300).description)
    }
    
    private func makeChatList() -> [ChatListModel] {
        return [ChatListModel(chatID: UUID(uuidString: "578E8708-36DC-4820-86DF-4CB00A1EC8C8")!, title: "Chat 1", timestamp: Date(timeIntervalSince1970: 100).description),
                ChatListModel(chatID: UUID(uuidString: "CA654CF5-862E-4FDA-8856-35B67564A07B")!, title: "Chat 2", timestamp: Date(timeIntervalSince1970: 200).description),
        ChatListModel(chatID: UUID(uuidString: "73B44F3D-240E-4025-81E3-16B29B6333A7")!, title: "Chat 3", timestamp: Date(timeIntervalSince1970: 300).description)]
    }
