...
/*
 User:
 {
   ...
   // Three independent timestamps the client mirrors into its registration
   // table. Kept OUTSIDE the `otp` object so they survive verification —
   // clearing the live code must not erase the record of what happened.
   phoneSentAt:   null | epochMs,   ← phone submitted (register / phone change)
   otpCodeSentAt: null | epochMs,   ← code last issued (also moves on resend)
   registeredAt:  null | epochMs,   ← successful verification. The only proof.
...
const publicUser = (u) => ({
  ...
  status: u.status,
  phoneSentDate: u.phoneSentAt ?? null,
  otpCodeSentDate: u.otpCodeSentAt ?? null,
...
function issueOTP(user) {
  const code = newOTP();
  const now = Date.now();
  user.otp = { code, expiresAt: now + OTP_TTL_MS, attempts: 0 };
  user.otpCodeSentAt = now;     // outlives the code itself
  persist();
...
const server = createServer(async (req, res) => {
  ...
  if (phoneMatch && req.method === "PUT") {
    ...
    user.phone = phone;
    user.phoneSentAt = Date.now();   // the number was submitted again either way
    if (changed) {
  ...
  switch (path) {
    ...
    case "/register": {
      ...
      if (user) {
        ...
        user = {
          ...
          otpCodeSentAt: null,      // set by issueOTP below
          registeredAt: null,
          otp: null,
        };
        db.users.push(user);
    ...
    case "/register/resend": {
      ...
      const since = user.otpCodeSentAt ? Date.now() - user.otpCodeSentAt : Infinity;
